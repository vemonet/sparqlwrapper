...
class SPARQLWrapper(object):
    ...
    def __init__(
        ...
        **default** value is ``None``.
        ...
        :type agent: string
        :param use_httpx: Whether to use the httpx library instead of urllib. The **default** value is ``False``.
        :type use_httpx: bool
        """
        ...
        self.httpx_client = httpx.Client(follow_redirects=True) if use_httpx else None

        if use_httpx and not HTTPX_AVAILABLE:
            warnings.warn(
                "use_httpx is set to True but httpx library is not available. Using urllib instead.",
                RuntimeWarning,
            )
    ...
    def _httpx_query(self) -> Tuple["httpx.Response", str]:
        """Internal method to execute the query using httpx. Returns the httpx.Response
        object if use_httpx is True.

        :return: tuples with the httpx response plus the expected format.
        ...
        :raises httpx.HTTPStatusError: If the HTTP return code is different from ``400``, ``401``, ``404``, ``414``, ``500``.
        :raises RuntimeError: If the httpx client is not initialized.
        """
        if self.httpx_client is None:
            raise RuntimeError("httpx client is not initialized, but _httpx_query was called")
        headers = {
            "User-Agent": self.agent,
            "Accept": self._getAcceptHeader(),
        }
        if self.user and self.passwd:
            if self.http_auth == BASIC:
                ...
                warnings.warn("Digest authentication is not supported with httpx.", RuntimeWarning)

        headers.update(self.customHttpHeaders)

        if self.isSparqlUpdateRequest():
            url = self.updateEndpoint
            params = None

            if self.requestMethod == POSTDIRECTLY:
                params = self.parameters  # Use parameters as URL query params
                data = self.queryString.encode("UTF-8")
                headers["Content-Type"] = "application/sparql-update"
            else:  # URL-encoded
                data = self._getRequestEncodedParameters(("update", self.queryString)).encode("ascii")
            ...
            if self.method == POST:
                if self.requestMethod == POSTDIRECTLY:
                    ...
                    params = None
                    data = self._getRequestEncodedParameters(("query", self.queryString)).encode("ascii")
                    headers["Content-Type"] = "application/x-www-form-urlencoded"
            else:  # GET
                params = None
                data = None
                # Append query parameters to URL
                url += "?" + self._getRequestEncodedParameters(("query", self.queryString))

        try:
            if params:
                # Convert parameters dict to proper format for httpx
                formatted_params = {}
                for key, values in params.items():
                    formatted_params[key] = values[0] if len(values) == 1 else values
            else:
            ...
            if e.response.status_code == 400:
                raise QueryBadFormed(e.response.text)
            elif e.response.status_code == 404:
                raise EndPointNotFound(e.response.text)
            elif e.response.status_code == 401:
                raise Unauthorized(e.response.text)
            elif e.response.status_code == 414:
                raise URITooLong(e.response.text)
            elif e.response.status_code == 500:
                raise EndPointInternalError(e.response.text)
            else:
                raise e

    def _query(self) -> Tuple[Union[HTTPResponse, "httpx.Response"], str]:
        ...
        or a httpx.Response object if use_httpx is True
...
class QueryResult(object):
    ...
    * ``info()`` that returns the meta-information of the HTTP result as a dictionary-like object.
    ...
    def info(self) -> KeyCaseInsensitiveDict[str]:
        ...
        elif hasattr(self.response, "headers"):
            # For httpx responses
            return KeyCaseInsensitiveDict(dict(self.response.headers))
        return KeyCaseInsensitiveDict({})
